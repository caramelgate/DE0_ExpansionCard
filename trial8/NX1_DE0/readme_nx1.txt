...
ビデオ回路を交換。縦横倍角など未実装。互換性は非常に低下。
すでにNISE X1の移植というより、パチモン改造品。
GRAMはまともに動作しそう？SDRAMのクロックをあげないと1024x768は間に合わなさげ。
そろそろ映像入力の任意位置にオーバーレイ(スーパーインポーズ)できる気がする。
手元にUSBキーボードしかないんで、サブCPUの動きが正しいかどうか不明。
...
ベースクロックは32MHzで、4MHzのZ80互換バスを用意。
いわゆるCPUローカルバスとI/Oバスを分離っていう手法を採用。
SDRAMクロックを64MHzに更新。
合成はできるが、デバッグはこれから。
